...
const EStatement = (props: { stage: string }) => {
    ...
    const getCurrentStage = async () => {
        await fetchNext(props.stage).then( async (response) => {
            if(response.data.data.task_name === "cbmScreen" || response.data.data.task_name == "cbmCompleted"){
                const task_id = response.data.data.task_id
                // complete task
                await complete(task_id, {"cbm_journey_completed": true}).then((response: any) => {
                    getVariable()
                })
            } else {
                getVariable()
